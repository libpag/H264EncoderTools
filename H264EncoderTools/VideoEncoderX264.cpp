...
class XXFrame {
private:
  XXFrame(int width, int height) : width(width), height(height) {
   stride[0] = (width + 15) & ~0x0F;
   stride[1] = stride[0] / 2;
   stride[2] = stride[0] / 2;
   
   data[0] = new uint8_t[stride[0]*height];
   data[1] = new uint8_t[stride[1]*((height+1)/2)];
   data[2] = new uint8_t[stride[2]*((height+1)/2)];
 }
public:
 int width = 0;
 int height = 0;
...
class XXEncoder {
 private:
  XXEncoder(int width, int height) : width(width), height(height) {
  }
 public:
  int width = 0;
  int height = 0;
  XXFrame* frame = nullptr;
};

void XXEncoderPlane(uint8_t* data, int width, int height, int stride) {
}
*/
